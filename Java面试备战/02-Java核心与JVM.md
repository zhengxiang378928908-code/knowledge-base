...
# 2. 如果机器上有多个 Java 进程，用 jps 确认具体应用
jps -l
...
> 我处理 CPU 飙高时，一般先用 `top` 看整机，再定位高 CPU 进程；如果机器上跑了多个 Java 服务，会用 `jps -l` 确认具体是哪一个应用。接着用 `top -H -p <pid>` 找出最耗 CPU 的线程，把线程 ID 转成十六进制后，用 `jstack` 看对应线程堆栈。然后再判断到底是死循环、频繁 GC、锁竞争，还是某段热点代码导致的。

**常见判断思路：**

- **业务线程打满**：通常能在栈里看到某个方法反复执行，常见是死循环、错误重试、复杂计算没收敛
- **GC 线程打满**：要继续结合 GC 日志、堆使用和对象分配速率看是不是 Full GC 频繁
- **锁竞争严重**：线程栈里会看到大量 `BLOCKED`、`waiting for monitor entry` 或反复争锁

**回答时最好补一句：**

不要只停留在“找到线程”这一步，最后一定要落到代码层，说明是哪个方法、什么原因、怎么改，比如改掉死循环、缩小锁粒度、减少对象创建，或者把全量处理改成分批处理。
